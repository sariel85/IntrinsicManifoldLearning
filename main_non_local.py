...
n_neighbors_cov = 100 #How neighboors to use from which to infer dynamics locally
...
dist_mat_local_trimmerd = numpy.array(dist_mat_local, copy=True)
...
'''
mds = manifold.MDS(n_components=2, max_iter=2000, eps=1e-7, dissimilarity="precomputed", n_jobs=1, n_init=1)
...
iso_embedding_local = mds.fit(dist_mat_local_trimmerd, weight=(dist_mat_local_trimmerd!=0).astype(int), init=iso_embedding_local).embedding_

#iso_embedding_true_sp = mds.fit(dist_mat_true_sp_temp, weight=dist_mat_true_sp_wgt).embedding_
iso_embedding_true_sp = mds.fit(dist_mat_true_sp_geo).embedding_

iso_embedding_measured_sp = mds.fit(dist_mat_measured_sp_geo).embedding_
...
diff_embedding_true_sp = calc_diff_map(dist_mat_true_sp_geo, dims=2, factor=2)
diff_embedding_measured_sp = calc_diff_map(dist_mat_measured_sp_geo, dims=2, factor=2)
...
print_process(diff_embedding_measured_sp.T, bounding_shape=None, color_map=color_map_clusters, titleStr="Regular Diffusion Maps")
stress, stress_normlized = embbeding_score(intrinsic_process_clusters, diff_embedding_measured_sp.T, titleStr="Regular Diffusion Maps", n_points=n_points_used_for_clusters)
