...
def get_metrics_from_net(non_local_tangent_net, noisy_sensor_clusters):
    ...
    for i_point in range(0, n_points):
        jacobian = non_local_tangent_net.get_jacobian_val(noisy_sensor_clusters[:, i_point].reshape((dim_measurement, 1)))[0][0, :, :]
        jacobian_int = non_local_tangent_net.get_jacobian_int_val(noisy_sensor_clusters[:, i_point].reshape((dim_measurement, 1)))[0][0, :, :]
