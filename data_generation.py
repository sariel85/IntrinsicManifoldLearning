...
def print_process(input_process, indexs=None, bounding_shape=None, color_map=None, ax=None, titleStr=None, covs=None, ax_limits=None, align_points=None):

    if indexs is None:
       indexs = numpy.arange(0, input_process.shape[1])
    else:
        color_map = color_map[indexs, :]

    input_process = input_process[:, indexs]
    ...
    if process.shape[0] == 2:
        ...
        if color_map is None:
            ...
            ax.scatter(process[0, :], process[1, :], c=color_map)
        ...
        if bounding_shape is not None:
            if bounding_shape.shape_type == 'Box':
                bounding_shape_vertices = numpy.asarray([bounding_shape.origin, bounding_shape.origin+[bounding_shape.scale[0], [0]], bounding_shape.origin + bounding_shape.scale, bounding_shape.origin+[[0], bounding_shape.scale[1]]], dtype=numpy.float32).squeeze()
        ...
        if color_map is None:
            ax.scatter(process[0, :], process[1, :], process[2, :])
        else:
            ax.scatter(process[0, :], process[1, :], process[2, :], c=color_map)
...
class BoundingShape(object):
    def __init__(self, vertices='nan', tri='nan', radius='nan', size='nan', predef_type='Not Specified', k=5, r_in=0.5, r_out=1, origin=[0, 0]):
        ...
        if predef_type == '1D Unit Segment':
            self.shape_type = "Box"
            self.origin = numpy.asarray([[0]], dtype=numpy.float32)
            self.scale = numpy.asarray([[1]], dtype=numpy.float32)
            self.dim = 1
            ...
            self.shape_type = "Box"
            self.origin = numpy.asarray([[0], [0], [0]], dtype=numpy.float32)
            self.scale = numpy.asarray([[1], [1], [1]], dtype=numpy.float32)
            self.dim = 3
    ...
    def dist_from_point(self, point):
        ...
        if self.shape_type == 'Box':
            dx = numpy.amax(numpy.concatenate((self.origin - point.reshape([self.dim, 1]), numpy.zeros([self.dim, 1], dtype=numpy.float32), point.reshape([self.dim, 1]) - (self.origin + self.scale)), axis=1), axis=1)
            ...
            poly_limit = PolygonShapely(self.vertices)
            return poly_limit.distance(Point(point[0], point[1]))
            ...
            if limit_polyhedron.winding_number((float(point[0]), float(point[1]), float(point[2]))) == 0:
                ...
                for i_tri in range(0, n_tri):
                    a = numpy.asarray(self.vertices[self.tri[i_tri][0]], dtype=numpy.float32)
                    b = numpy.asarray(self.vertices[self.tri[i_tri][1]], dtype=numpy.float32)
                    c = numpy.asarray(self.vertices[self.tri[i_tri][2]], dtype=numpy.float32)
                    vect_a = b - a
                    ...
                    mat = numpy.asarray([vect_b1.reshape(3), vect_b2.reshape(3)], dtype=numpy.float32).T

                    vect_a_2d = numpy.dot(mat.T, vect_a)
                    vect_b_2d = numpy.dot(mat.T, vect_b)
                    vect_o_2d = numpy.asarray([0, 0], dtype=numpy.float32)
